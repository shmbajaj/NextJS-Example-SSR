...
export class AuthLiteClient {
   ...
   */
  private isPromiseResolved: boolean;
  ...
  constructor(apiKey: string, secretKey: string, orgId?: string) {
     ...
     */
    console.log(this.jwtEncode(this.secretKey, { api_key: this.apiKey }));
  }
  ...
  async getUser(token: string): Promise<GetUser> {
    ...
    try {
      /**
       * Below  code checks isPromise resolved, if not then throws the error
       */
      console.log({
        isPromiseResolved: this.isPromiseResolved,
