...
export class AuthLiteClient {
  ...
  public async init() {
    this.signedKey = await this.jwtEncode(this.secretKey, {
      api_key: this.apiKey,
    });
    console.log(this);
  }
  ...
  private async jwtEncode(key: string, data: JWTPayload) {
    ...
    return await new SignJWT(data).setProtectedHeader({ alg }).sign(secret);
  }
  ...
  async getUser(token: string): Promise<GetUser> {
    ...
    try {
      /**
       * Below code checks isSignedKey resolved, if not then throws the error
       */
      console.log({ signedKey: this.signedKey });
      if (!this.signedKey) {
