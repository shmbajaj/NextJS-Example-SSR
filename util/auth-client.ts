...
import { isResolved } from './promise';
...
export class AuthLiteClient {
  ...
  /**
   * Temporarily added to prove promise is resolved before `getUser()` is called
   */
  ...
  constructor(apiKey: string, secretKey: string, orgId?: string) {
    ...
    this.orgId = orgId;
    /**
     * This console log is expected to log undefined and Promise<pending>
     * Because at this point of js engine, promise code is sent to microtask queue
     * Later when `getUser()` is called, by then promise is resolved
     */
    this.promise = this.jwtEncode(this.secretKey, { api_key: this.apiKey });
  }
  ...
  async getUser(token: string): Promise<GetUser> {
    ...
    try {
      /**
       * Below  function checks isPromise resolved, if not then throws the error
       */
      const result = await isResolved(this.promise);
      console.log({ result, promise: this.promise, signedKey: this.signedKey });
      if (!result) {
